...
function statusOf(disabled, fiber) {
    if (disabled)
        return 'disabled';
...
export class McpBackend {
        ...
        for (const entry of loader.entries()) {
            ...
            for (const name of tools)
                if (name.startsWith(prefix))
                    serverTools.push(name.slice(prefix.length));
            serverTools.sort();
        ...
        for (const patch of contents.items) {
            ...
            for (let index = 0; index < insert.items.length; index += 1) {
                ...
                if (!(entry instanceof YAMLMap))
                    continue;
                if (entry.get('id', false) === id)
                    return { entry, seq: insert, index, patch };
            }
        }
        return undefined;
    }
    ...
    insertTarget(doc) {
        const contents = doc.contents;
        if (!(contents instanceof YAMLSeq))
            throw new Error('profile patch file is not a YAML array');
        let firstInsert;
        for (const patch of contents.items) {
            if (!(patch instanceof YAMLMap))
                continue;
            ...
            for (const entry of insert.items) {
                if (entry instanceof YAMLMap && entry.get('name', false) === MCP_MODULE)
                    return insert;
            }
        }
        ...
        const fresh = doc.createNode({ insert: [] });
        contents.add(fresh);
        const insert = fresh.get('insert', true);
        if (!(insert instanceof YAMLSeq))
            throw new Error('failed to create an insert section in the profile patch file');
        return insert;
    }
    entryNode(doc, input) {
        const entry = {
    ...
    async save(input) {
        ...
        const serverNameTaken = (this.snapshot().servers.some(server => server.serverName === input.serverName && server.id !== input.id));
        if (serverNameTaken)
            throw new Error(`服务器名称 ${input.serverName} 已被其他实例使用`);
        await this.mutatePatchFile((doc) => {
            ...
            if (found !== undefined) {
                ...
                found.seq.items[found.index] = this.entryNode(doc, input);
            }
            else {
    ...
    async remove(id) {
        await this.mutatePatchFile((doc) => {
            ...
            if (found === undefined)
                throw new Error(`MCP server instance not found in patch file: ${id}`);
            ...
            if (contents instanceof YAMLSeq) {
                ...
                if (found.seq.items.length === 0 && patchIndex >= 0)
    ...
    async setEnabled(id, enabled) {
        await this.mutatePatchFile((doc) => {
            ...
            if (found === undefined)
                throw new Error(`MCP server instance not found in patch file: ${id}`);
            if (enabled)
                found.entry.delete('disabled');
            else
                found.entry.set('disabled', true);
        });
    }
    ...
    async handle(req, res) {
        ...
        try {
            if (request.action === 'remove')
                ...
                await this.setEnabled(request.id, request.enabled);
            else
                await this.save(request.server);
            json(res, 200, { ok: true, value: this.snapshot() });
        }
        catch (error) {
...
function buildConfig(input) {
    ...
    if (input.toolCallTimeoutMs !== undefined)
        base.toolCallTimeoutMs = input.toolCallTimeoutMs;
    if (input.transport === 'streamable-http') {
        return {
            ...base,
            url: input.url ?? '',
...
function parseYamlConfig(source) {
    ...
    if (!isRecord(raw))
        throw new TypeError('YAML 必须是对象（一段 MCP 配置）');
    ...
    if (transport === 'streamable-http') {
        if (typeof raw.url !== 'string' || !/^https?:\/\//u.test(raw.url))
            throw new TypeError('url 必须以 http:// 或 https:// 开头');
        return {
    ...
    if (typeof raw.command !== 'string' || raw.command.trim().length === 0)
        throw new TypeError('command 不能为空');
    return {
        serverName: raw.serverName,
        transport,
        command: raw.command,
    ...
    if (value.action === 'save') {
        ...
        if (typeof value.yaml === 'string') {
            ...
            if (typeof idRaw !== 'string' || idRaw.length === 0)
                throw new TypeError('save (yaml) requires server.id');
 ...
 */
export function installMcpWeb(ctx, backend) {
