...
import { sameOriginPost } from "./web.js";
...
export class GitWebHandler {
        ...
        if (req.method === 'GET') {
            ...
            try {
                ...
                if (path === `${GIT_ROUTE}/diff`) {
                    if (file === undefined)
                        throw new TypeError('file query parameter is required');
                    if (mode !== 'hunk' && mode !== 'full')
                        throw new TypeError('mode must be "hunk" or "full"');
        ...
        try {
            if (path === `${GIT_ROUTE}/commit`) {
                ...
                const exclude = isRecord(body) && Array.isArray(body.exclude)
                    ? body.exclude.filter((entry) => typeof entry === 'string' && entry !== '')
                    : undefined;
    ...
    requireCwd(target) {
        ...
        if (cwd === undefined)
            throw new Error('no working directory for this session');
        return cwd;
    }
    /** Serve one whitelisted vendored asset; anything else is a 404. */
    serveVendor(res, name) {
...
function terminalKeyOf(session, ws) {
 ...
 */
export function installGitWeb(ctx, backend) {
...
function attachTerminalSocket(backend, key, cwd, dims, ws) {
    ...
    const client = {
        ...
        get alive() { return ws.readyState === WebSocket.OPEN; },
    };
    let session;
    try {
...
//# sourceMappingURL=git-web.js.map