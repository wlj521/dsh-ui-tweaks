...
function parseStatusHeader(stdout) {
    const first = stdout.split('\0', 1)[0] ?? '';
    if (!first.startsWith('## '))
        return { branch: null, detached: false, ahead: 0, behind: 0 };
    ...
    const out = { branch: local !== '' ? local : null, detached: false, ahead: 0, behind: 0 };
    if (upstreamRef !== undefined && upstreamRef !== '')
        out.upstream = upstreamRef;
    for (const piece of tracking.split(',')) {
        const match = piece.trim().match(/^(ahead|behind)\s+(\d+)$/);
        if (match === null)
            continue;
        if (match[1] === 'ahead')
            out.ahead = Number(match[2]) || 0;
        else
            out.behind = Number(match[2]) || 0;
    }
    return out;
}
...
function mergeNumstat(map, stdout) {
    for (const [path, value] of parseNumstat(stdout))
        map.set(path, value);
}
/** Resolve a repo-relative path safely: must stay inside the cwd. */
function resolveWithin(cwd, path) {
...
async function countFileLines(cwd, path) {
    try {
        const full = resolveWithin(cwd, path);
        const info = await stat(full);
        if (!info.isFile() || info.size > MAX_COUNT_FILE_BYTES)
            return 0;
...
function readFileContent(cwd, path, signal) {
    return (async () => {
        const full = resolveWithin(cwd, path);
        if (existsSync(full)) {
...
function addedNewLineNumbers(diff) {
    const out = new Set();
    for (const line of diff) {
        if (line.type === 'add' && line.new !== null)
            out.add(line.new);
    }
    return out;
}
...
export class GitBackend {
    ...
    async snapshot(cwd, signal) {
        ...
        if (worktreeNumstat !== undefined)
            mergeNumstat(numstat, worktreeNumstat.stdout);
        if (stagedNumstat !== undefined)
            mergeNumstat(numstat, stagedNumstat.stdout);
        const files = [];
     ...
     */
    async diff(cwd, path, mode, signal) {
        if (path.includes('\0'))
            throw new Error('invalid path');
        if (mode !== 'hunk' && mode !== 'full')
        ...
        if (rawLines.length > 0 && rawLines[rawLines.length - 1] === '')
            rawLines.pop();
        const lines = rawLines.slice(0, MAX_FULL_LINES).map((text, i) => ({
    ...
    async commit(cwd, message, opts = {}) {
        ...
        if (exclude.length === 0) {
            ...
            if (included.length === 0)
                throw new Error('no files to commit — everything is excluded');
        ...
        if (opts.push === true) {
            ...
            pushed = true;
            if (tagCreated !== undefined)
                await this.pushTag(cwd, tagCreated, opts.signal);
        }
        const result = { pushed };
        if (hash !== undefined)
            result.hash = hash;
        if (tagCreated !== undefined)
            result.tag = tagCreated;
        return result;
    }
    /** Delete a local branch (force; the UI confirms first). Protected branches cannot be deleted. */
    async deleteBranch(cwd, name, signal) {
     ...
     */
    async deleteRemoteBranch(cwd, name, signal) {
     ...
     */
    async renameBranch(cwd, name, newName, signal) {
        if (!isValidBranchName(name) || !isValidBranchName(newName))
            throw new Error('invalid branch name');
        if (isProtectedBranchName(name))
            throw new Error(`cannot rename protected branch: ${name}`);
        if (name === newName)
            return;
        await runGit(cwd, ['branch', '-m', name, newName], { signal });
    }
    /** Tags newest-first, each with the short hash + subject of its commit. */
    async tags(cwd, signal) {
     ...
     */
    async graph(cwd, limit, signal) {
        const n = Math.max(1, Math.min(500, Number.isFinite(limit) ? Math.floor(limit) : 150));
        const sep = '\u001f';
        const { stdout } = await runGit(cwd, [
        ...
        for (const line of stdout.split('\n')) {
            ...
            if (fullHash === undefined || fullHash === '' || subject === undefined)
                continue;
            commits.push({
     ...
     */
    async createBranch(cwd, name, base, pushRemote, signal) {
        if (!isValidBranchName(name))
            throw new Error('invalid branch name');
...
export class TerminalSessionManager {
    sessions = new Map();
    nodePty;
    constructor(loadPty = loadNodePty) {
     ...
     */
    attach(key, cwd, cols, rows, client) {
    ...
    detach(key, client) {
        ...
        if (session.clients.size === 0 && !session.exited)
            this.scheduleClose(key, SOCKET_DROP_GRACE_MS);
    }
    ...
    spawn(key, cwd, cols, rows) {
        if (this.nodePty === undefined)
        ...
        const session = { key, cwd, pty, transcript: '', exited: false, exitCode: null, clients: new Set(), closeTimer: null };
        pty.onData((data) => {
...
function isAbortError(error) {
    return error !== null && typeof error === 'object' && 'aborted' in error && error.aborted === true;
}
