...
.duwi-row{box-sizing:border-box;width:100%;max-width:calc(var(--dsh-composer-card-max-width) + 2 * var(--dsh-composer-side-clearance, 0px));margin:0 auto;padding:0 var(--dsh-composer-side-clearance, 0px);display:flex;align-items:flex-end;justify-content:flex-end;height:26px;pointer-events:none;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none}
...
export function installWhaleStyles(): () => void {
 ...
 */
function liftLiveWhalePath(): string | undefined {
  const path = document.querySelector<SVGPathElement>('[class*="brandMark"] svg path')
...
export function WhaleIndicator({ sessionId, input, sessionsService }: WhaleIndicatorProps) {
  // Prefer the live sidebar whale when one is on screen; fall back to stock.
  const [path, setPath] = useState(STOCK_WHALE_PATH)
  useEffect(() => {
  ...
  const subscribe = useMemo(() => list.subscribe.bind(list), [list])
  ...
  return (
    <div className={swim ? 'duwi-row duwi-swim' : 'duwi-row'} aria-hidden="true">
      <div className="duwi-stack">
        <svg
          className="duwi-whale"
          width={WHALE_WIDTH}
          height={WHALE_WIDTH * WHALE_VIEW_H / WHALE_VIEW_W}
          viewBox={`0 0 ${WHALE_VIEW_W} ${WHALE_VIEW_H}`}
          fill="none"
          onClick={swimFor}
        >
          <path d={path} fill="currentColor" />
        </svg>
        <svg className="duwi-waves" viewBox="0 0 26 9" fill="none">
          {/* Wavelength-8 sine strips drawn from -8 to +40 so the ±8px drift loop stays covered. */}
          <path className="duwi-wave-back" d="M-8 4 Q-4 5.8 0 4 T8 4 T16 4 T24 4 T32 4 T40 4" strokeWidth="1.4" strokeLinecap="round" />
          <path className="duwi-wave-front" d="M-8 5.5 Q-4 3.3 0 5.5 T8 5.5 T16 5.5 T24 5.5 T32 5.5 T40 5.5" strokeWidth="1.4" strokeLinecap="round" />
        </svg>
      </div>
    </div>
  )
