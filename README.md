...
- **主题（布局区单选，默认即原生外观）**：可选 `默认` 或 **荧光青**——深色底 + 荧光青点缀（按钮、链接、选中态跟随主题变量自动变色，代码块带一圈青色辉光），切换即时生效。后续皮肤（品红、黄黑等）会以同样方式追加为新选项。
...
ui-tweaks:
  ...
  themeStyle: neon-cyan         # 默认 default（DSH 原生外观），neon-cyan 为荧光青皮肤
